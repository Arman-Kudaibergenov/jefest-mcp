...
**Output location:** Write to `presentations/` in the current project directory (e.g., `presentations/modem-architecture.html`). Use a descriptive filename based on content. The folder is created by `1c-project-init` as part of the standard scaffold. If for some reason the folder doesn't exist, create it before writing.

**Open in browser:**
- Windows: `start "" "presentations\filename.html"`
